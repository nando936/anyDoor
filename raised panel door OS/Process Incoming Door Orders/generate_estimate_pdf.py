...
from shared_utils import load_unified_json
...
def generate_estimate_pdf(order_json_path, output_path, estimate_number=None):
    ...
    # Process doors from unified format - Group by size category
    ...
    for door in doors:
        marker = door['marker']
        qty = int(door['qty'])
        sqft = door['sqft']  # Already calculated in unified format
